        ...
        window.hotspotLogin = function({username,password,mk}){
            return {
                ...
                init(){
                    ...
                    if(this.mk['chap-id']){ this.finalPass = hexMD5(this.mk['chap-id'] + this.password + this.mk['chap-challenge']); }
                    else { this.finalPass = this.password; }
                    this.ready = true; this.submitting = true; setTimeout(()=>{ this.$refs.form.submit(); },150);
                }
            }
        }
    }
                    ...
                    <div class="relative space-y-6">
                        ...
                        @else
                            <div x-data="{show:false}" x-init="setTimeout(()=>show=true,40)" x-show="show" x-transition.opacity.duration.350ms class="space-y-6 anim-fade-scale">
                                ...
                                @if(!$preview && (!isset($attempt->offline) || !$attempt->offline))
                                    <div class="space-y-2" x-data="window.hotspotLogin({ username: @js($credentials['username']), password: @js($credentials['password']), mk: @js($mikrotik) })" x-init="init()">
                                        <template x-if="!ready">
                                        ...
                                        <!-- hotspot auto-login script global definido arriba -->
                                    </div>
                                @elseif($preview)
